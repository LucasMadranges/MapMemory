    ...
    <div className={containerClassName}>
      ...
      <div className={'relative'}>
        <input
          ...
          value={value}
          type={typeInput}
          className={`border border-gray-300 rounded-lg w-full p-2 ${icon && 'pr-10'} ${
            type === 'password' && 'pr-10'
          } focus-visible:outline-0 ${inputClassName}`}
            ...
            <>
              {isPasswordHidden && (
                <Button
                  type={'tertiary'}
                  className={`p-1`}
                  onClick={(event) => handleEnabledPassword(event)}
                >
                  {disabledPasswordIcon}
                </Button>
              )}
              {!isPasswordHidden && (
                <Button
                  type={'tertiary'}
                  className={`p-1`}
                  onClick={(event) => handleDisabledPassword(event)}
                >
                  {enabledPasswordIcon}
                </Button>
              )}
